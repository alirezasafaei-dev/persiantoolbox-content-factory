...
class MetaInstagramSettings:
    ...
    host_url: str = field(default_factory=lambda: _env("META_HOST_URL", "https://graph.facebook.com"))

    # Publish controls
    auto_publish: bool = field(default_factory=lambda: _env_bool("PTB_AUTO_PUBLISH", False))
    live_canary_approved: bool = field(default_factory=lambda: _env_bool("PTB_LIVE_CANARY_APPROVED", False))
    generation_enabled: bool = field(default_factory=lambda: _env_bool("PTB_GENERATION_ENABLED", True))
    scheduler_enabled: bool = field(default_factory=lambda: _env_bool("PTB_SCHEDULER_ENABLED", False))
    real_publish_enabled: bool = field(default_factory=lambda: _env_bool("PTB_REAL_PUBLISH_ENABLED", False))
    ...
    container_timeout_seconds: int = field(default_factory=lambda: _env_int("PTB_CONTAINER_TIMEOUT", 600))
    ...
    media_gateway_ttl_hours: int = field(default_factory=lambda: _env_int("PTB_MEDIA_GATEWAY_TTL_HOURS", 24))
    media_gateway_host: str = field(default_factory=lambda: _env("PTB_MEDIA_GATEWAY_HOST", ""))

    # Queue
    queue_db_path: str = field(default_factory=lambda: _env("PTB_QUEUE_DB", "data/publish-queue.db"))

    # OAuth redirect
    oauth_redirect_uri: str = field(default_factory=lambda: _env("PTB_OAUTH_REDIRECT_URI", "http://localhost:8080/callback"))
    ...
    def has_credentials(self) -> bool:
        return bool(self.app_id and self.app_secret and self.access_token and self.instagram_account_id)
